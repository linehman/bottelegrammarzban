...
#-----------telegram_ip_ranges------------#
    ...
    if ($text == "✅ قوانین را می پذیرم"){
        sendmessage($from_id,$textbotlang['users']['Rules'], $keyboard);
    ...
    if ($text == "🏠 بازگشت به منوی اصلی") {
        $textback = $textbotlang['users']['back'];
    ...
    if($user['step'] == 'get_number'){
        if (empty($user_phone)){
            sendmessage($from_id,$textbotlang['users']['number']['false'], $request_contact);
            return;
        }
        if ($contact_id != $from_id){
            sendmessage($from_id, $textbotlang['users']['number']['Warning'],$request_contact );
            return;
        }
        if($setting['iran_number'] == "✅ احرازشماره ایرانی روشن است" && !preg_match("/989[0-9]{9}$/",$user_phone)){
                    sendmessage($from_id,$textbotlang['users']['number']['erroriran'],$request_contact);
                    return;
        }
        sendmessage($from_id, $textbotlang['users']['number']['active'], $keyboard);
    ...
    if ($text == $datatextbot['text_Purchased_services'] || $datain == "backorder") {
        ...
        if(is_null($invoices) && $setting['NotUser'] == "offnotuser"){
             sendmessage($from_id, $textbotlang['users']['sell']['service_not_available'], null);
             return;
        }
    ...
    sendmessage($from_id, $textbotlang['users']['sell']['service_sell'], $keyboard_json);
    }
        if($setting['NotUser'] == "onnotuser"){
        sendmessage($from_id, $textbotlang['users']['stateus']['notUsername'], $NotProductUser);
        }
    }
    if($text == "⭕️ نام کاربری من در لیست نیست ⭕️"){
        sendmessage($from_id, $textbotlang['users']['stateus']['SendUsername'], $backuser);
        $stmt = $connect->prepare("UPDATE user SET step = ? WHERE id = ?");
    ...
    if ($user['step'] == "getusernameinfo") {
        if (!preg_match('/^\w{3,32}$/', $text)) {
            $textusernameinva = $textbotlang['users']['stateus']['Invalidusername'];
        ...
        $stmt->execute();
        sendmessage($from_id, $textbotlang['users']['Service']['Location'], $list_marzban_panel_user);
        $stmt = $connect->prepare("UPDATE user SET step = ? WHERE id = ?");
        $step = 'getdata';
        $stmt->bind_param("ss", $step, $from_id);
        $stmt->execute();
    ...
    elseif ($user['step'] == "getdata") {
        ...
        if ($data_useer['detail'] == "User not found") {
            sendmessage($from_id,$textbotlang['users']['stateus']['notUsernameget'],$keyboard);
            $stmt = $connect->prepare("UPDATE user SET step = ? WHERE id = ?");
            $step = 'home';
            $stmt->bind_param("ss", $step, $from_id);
            $stmt->execute();
        ...
        sendmessage($from_id, $textbotlang['users']['stateus']['info'], $keyboardinfo);
        sendmessage($from_id,$textbotlang['users']['selectoption'], $keyboard);
        $stmt = $connect->prepare("UPDATE user SET step = ? WHERE id = ?");
        $step = 'home';
        $stmt->bind_param("ss", $step, $from_id);
        $stmt->execute();
    ...
    if(preg_match('/product_(\w+)/',$datain, $dataget)) {
        ...
        if ($data_useer['detail'] == "User not found") {
            sendmessage($from_id, $textbotlang['users']['stateus']['error'], $keyboard);
            return;
        }
        #-------------username----------------#
...
$status = $data_useer['status'];
    ...
    if ($text == $datatextbot['text_usertest']) {
        if ($user['limit_usertest'] == 0) {
            sendmessage($from_id, $textbotlang['users']['usertest']['limitwarning'] , $keyboard);
            return;
        }
        sendmessage($from_id, $textbotlang['users']['Service']['Location'], $list_marzban_panel_user);
        $stmt = $connect->prepare("UPDATE user SET step = ? WHERE id = ?");
        ...
        $data_test = json_decode($config_test, true);
            if(!isset($data_test['username'])){
             if(isset($data['detail']['proxies']))$data['detail'] = $data['detail']['proxies'];
                sendmessage($from_id,$textbotlang['users']['usertest']['errorcreat'] ,$keyboard);
        $texterros = "
   ...
   {$textbotlang['users']['config']}
    ...
    if ($text == $datatextbot['text_help']) {
        if ($setting['help_Status'] == "❌ آموزش غیرفعال است"){
            sendmessage($from_id, $textbotlang['users']['help']['disablehelp'], null);
            return;
        }
        sendmessage($from_id, $textbotlang['users']['selectoption'], $json_list_help);
        $stmt = $connect->prepare("UPDATE user SET step = ? WHERE id = ?");
        $step = 'sendhelp';
        $stmt->bind_param("ss", $step, $from_id);
        $stmt->execute();
    ...
    if ($text == $datatextbot['text_support']) {
        ...
        sendmessage($from_id, $textbotlang['users']['support']['sendmessageadmin'], $keyboard);
        $textsendadmin = "
    ...
    if ($text == $datatextbot['text_sell']) {
        if ($setting['get_number'] == "✅ تایید شماره موبایل روشن است" && $user['step'] != "get_number" && $user['number'] == "none"){
            sendmessage($from_id, $textbotlang['users']['number']['Confirming'] , $request_contact);
            $stmt = $connect->prepare("UPDATE user SET step = ? WHERE id = ?");
            $step = 'get_number';
            $stmt->bind_param("ss", $step, $from_id);
            $stmt->execute();
    ...
    elseif ($user['step'] == "get_product"){
        if (!in_array($text , $marzban_list)){
            sendmessage($from_id, $textbotlang['users']['sell']['Service-Location'], null);
            return;
        }
        $stmt = $connect->prepare("UPDATE user SET Processing_value = ? WHERE id = ?");
        $stmt->bind_param("ss", $text, $from_id);
        $stmt->execute();
        sendmessage($from_id,$textbotlang['users']['sell']['Service-select'], $json_list_product_list);
        $stmt = $connect->prepare("UPDATE user SET step = ? WHERE id = ?");
        $step = 'endstepuser';
        $stmt->bind_param("ss", $step, $from_id);
        $stmt->execute();
    ...
    elseif ($user['step'] == "payment" && $text == "💰 پرداخت و دریافت سرویس"){
        ...
        if ($info_product['price_product'] > $user['Balance']){
            sendmessage($from_id,$textbotlang['users']['sell']['None-credit'], $keyboard);
            $stmt = $connect->prepare("UPDATE user SET step = ? WHERE id = ?");
            $step = 'home';
            $stmt->bind_param("ss", $step, $from_id);
            $stmt->execute();
   ...
   {$textbotlang['users']['stateus']['getlinksub']}
    ...
    if($text == $datatextbot['text_Add_Balance']){
        ...
        if ($user['number'] == "none" && $setting['get_number'] == "✅ تایید شماره موبایل روشن است" ) return;
        sendmessage($from_id, $textbotlang['users']['Balance']['priceinput'], $backuser);
        $stmt = $connect->prepare("UPDATE user SET step = ? WHERE id = ?");
        $step = 'getprice';
        $stmt->bind_param("ss", $step, $from_id);
        $stmt->execute();
    ...
    elseif ($user['step'] == "getprice"){
        if (!ctype_digit($text)){
            sendmessage($from_id,$textbotlang['users']['Balance']['errorprice'], null);
            return;
        }
        if(intval($text)>10000000){
            sendmessage($from_id, $textbotlang['users']['Balance']['errorpricelimit'],  null);
            return;
        }
        $stmt = $connect->prepare("UPDATE user SET Processing_value = ? WHERE id = ?");
        $stmt->bind_param("ss", $text, $from_id);
        $stmt->execute();
        sendmessage($from_id,$textbotlang['users']['Balance']['selectPatment'], $step_payment);
        $stmt = $connect->prepare("UPDATE user SET step = ? WHERE id = ?");
    ...
    elseif($user['step'] == "get_step_payment"){
        ...
        $usdprice = round($Processing_value/$USD,2);
            if($usdprice < 2){
            sendmessage($from_id, $textbotlang['users']['Balance']['nowpayments'], null);
    return;
            }
            sendmessage($from_id, $textbotlang['users']['Balance']['linkpayments'], $keyboard);
    ...
    ";
        ...
        $usdprice = round($Processing_value / $usd,2);
            if($trxprice <= 1){
            sendmessage($from_id,$textbotlang['users']['Balance']['weswap'], null);
    return;
            }
        sendmessage($from_id, $textbotlang['users']['Balance']['linkpayments'], $keyboard);
        $dateacc = date('Y/m/d h:i:s');
        ...
        if(!isset($pay->pay_address)){
            ...
            sendmessage($from_id,$textbotlang['users']['Balance']['errorLinkPayment'], $keyboard);
            $stmt = $connect->prepare("UPDATE user SET step = ? WHERE id = ?");
            $step = 'home';
            $stmt->bind_param("ss", $step, $from_id);
            $stmt->execute();
    ...
    if(preg_match('/Confirmpay_user_(\w+)_(\w+)/',$datain, $dataget)) {
        ...
        $stmt->execute();
        sendmessage($from_id, $textbotlang['users']['Balance']['Confirmpay'], null);
    ...
    elseif($user['step'] =="cart_to_cart_user"){
        if (!$photo){
            sendmessage($from_id, $textbotlang['users']['Balance']['Invalid-receipt'], null);
            return;
        }
        $dateacc = date('Y/m/d h:i:s');
        $randomString = bin2hex(random_bytes(5));
        ...
        $stmt->bind_param("sssss", $from_id ,$randomString, $dateacc,$Processing_value,$payment_Status);
        $stmt->execute();
        sendmessage($from_id,$textbotlang['users']['Balance']['Send-receipt'], $keyboard);
    ...
    if($text == $datatextbot['text_Discount']){
        sendmessage($from_id, $textbotlang['users']['Discount']['getcode'], $backuser);
        $stmt = $connect->prepare("UPDATE user SET step = ? WHERE id = ?");
        $step = 'get_code_user';
        $stmt->bind_param("ss", $step, $from_id);
        $stmt->execute();
    ...
    elseif($user['step'] == "get_code_user"){
        if(!in_array($text , $Discouncode)){
                sendmessage($from_id, $textbotlang['users']['Discount']['notcode'], null);
                return;
        }
    ...
    $Checkcode = [];
        ...
        sendmessage($from_id,$textbotlang['users']['Discount']['onecode'], $keyboard);
        $stmt = $connect->prepare("UPDATE user SET step = ? WHERE id = ?");
        $step = 'اhome';
        $stmt->bind_param("ss", $step, $from_id);
        $stmt->execute();
    ...
    if($text == $datatextbot['text_Account_op']){
            sendmessage($from_id,$textbotlang['users']['selectoption'], $keyboardPanel);
    }
    #----------------[  admin section  ]------------------#
    $textadmin = ["panel","/panel","پنل مدیریت","ادمین"];
    if (!in_array($from_id, $admin_ids)){ 
        if(in_array($text, $textadmin)){
        sendmessage($from_id, $textbotlang['users']['Invalid-comment'], null);
        foreach($admin_ids as $admin){
            $textadmin = "
    ...
    if ($text == "🏠 بازگشت به منوی مدیریت") {
        sendmessage($from_id, $textbotlang['Admin']['Back-Admin'], $keyboardadmin);
        $stmt = $connect->prepare("UPDATE user SET step = ? WHERE id = ?");
        $step = 'home';
        $stmt->bind_param("ss", $step, $from_id);
        $stmt->execute();
    ...
    if ($text == "🔑 روشن / خاموش کردن قفل کانال") {
        if ($channels['Channel_lock'] == "off") {
            sendmessage($from_id, $textbotlang['Admin']['channel']['join-channel-on'], $channelkeyboard);
            $stmt = $connect->prepare("UPDATE channels SET Channel_lock = ?");
            $Channel_lock = 'on';
            $stmt->bind_param("s", $Channel_lock);
            $stmt->execute();
        } else {
            sendmessage($from_id,$textbotlang['Admin']['channel']['join-channel-off'] , $channelkeyboard);
            $stmt = $connect->prepare("UPDATE channels SET Channel_lock = ?");
            $Channel_lock = 'off';
            $stmt->bind_param("s", $Channel_lock);
            $stmt->execute();
    ...
    if ($text == "📣 تنظیم کانال جوین اجباری") {
        $text_channel = $textbotlang['Admin']['channel']['changechannel'].$channels['link'];
            telegram('sendmessage',[
            'chat_id' => $from_id,
            'text' => $text_channel,
    ...
    elseif ($user['step'] == "addchannel") {
        sendmessage($from_id, $textbotlang['Admin']['channel']['setchannel'], $channelkeyboard);
        $stmt = $connect->prepare("UPDATE user SET step = ? WHERE id = ?");
        $step = 'home';
        $stmt->bind_param("ss", $step, $from_id);
        $stmt->execute();
    ...
    if ($text == "👨‍💻 اضافه کردن ادمین") {
        sendmessage($from_id, $textbotlang['Admin']['manageadmin']['getid'], $backadmin);
        $stmt = $connect->prepare("UPDATE user SET step = ? WHERE id = ?");
        $step = 'addadmin';
        $stmt->bind_param("ss", $step, $from_id);
        $stmt->execute();
    ...
    if ($user['step'] == "addadmin") {
        sendmessage($from_id, $textbotlang['Admin']['manageadmin']['addadminset'], $keyboardadmin);
        $stmt = $connect->prepare("UPDATE user SET step = ? WHERE id = ?");
        $step = 'home';
        $stmt->bind_param("ss", $step, $from_id);
        $stmt->execute();
    ...
    if ($user['step'] == "deleteadmin") {
        ...
        sendmessage($from_id, $textbotlang['Admin']['manageadmin']['removedadmin'], $keyboardadmin);
    ...
    if ($text == "➕ محدودیت ساخت اکانت تست برای کاربر") {
        sendmessage($from_id, $textbotlang['Admin']['manageusertest']['getidlimit'], $backadmin);
        $stmt = $connect->prepare("UPDATE user SET step = ? WHERE id = ?");
    ...
    if ($user['step'] == "add_limit_usertest_foruser") {
        if (!in_array($text, $users_ids)) {
            sendmessage($from_id, $textbotlang['Admin']['not-user'], $backadmin);
            return;
        }
        sendmessage($from_id,$textbotlang['Admin']['getlimitusertest']['getid'],$backadmin);
        $stmt = $connect->prepare("UPDATE user SET Processing_value = ? WHERE id = ?");
        $stmt->bind_param("ss", $text, $from_id);
        $stmt->execute();
        $stmt = $connect->prepare("UPDATE user SET step = ? WHERE id = ?");
    ...
    if ($user['step'] == "get_number_limit") {
        sendmessage($from_id, $textbotlang['Admin']['getlimitusertest']['setlimit'], $keyboard_usertest);
        $id_user_set = $text;
        $stmt = $connect->prepare("UPDATE user SET step = ? WHERE id = ?");
        $step = 'home';
        $stmt->bind_param("ss", $step, $from_id);
    ...
    if ($text == "➕ محدودیت ساخت اکانت تست برای همه") {
        sendmessage($from_id,$textbotlang['Admin']['getlimitusertest']['limitall'],$backadmin);
        $stmt = $connect->prepare("UPDATE user SET step = ? WHERE id = ?");
    ...
    elseif ($user['step'] == "limit_usertest_allusers") {
        sendmessage($from_id, $textbotlang['Admin']['getlimitusertest']['setlimitall'], $keyboard_usertest);
        $stmt = $connect->prepare("UPDATE user SET step = ? WHERE id = ?");
        $step = 'home';
        $stmt->bind_param("ss", $step, $from_id);
        $stmt->execute();
    ...
    if ($text == "📯 تنظیمات کانال") {
        sendmessage($from_id, $textbotlang['users']['selectoption'] , $channelkeyboard);
    }
    #-------------------------#
    if ($text == "📊 آمار ربات") {
    ...
    if ($text == "🖥 پنل مرزبان") {
        sendmessage($from_id,$textbotlang['users']['selectoption'], $keyboardmarzban);
    }
    if ($text == "🔌 وضعیت پنل") {
        sendmessage($from_id, $textbotlang['Admin']['managepanel']['selectpanel'], $json_list_marzban_panel);
        $stmt = $connect->prepare("UPDATE user SET step = ? WHERE id = ?");
        $step = 'get_panel';
        $stmt->bind_param("ss", $step, $from_id);
        $stmt->execute();
    ...
    if ($text == "🖥 اضافه کردن پنل  مرزبان") {
        sendmessage($from_id, $textbotlang['Admin']['managepanel']['addpanelname'], $backadmin);
        $stmt = $connect->prepare("UPDATE user SET step = ? WHERE id = ?");
        $step = 'add_name_panel';
        $stmt->bind_param("ss", $step, $from_id);
        $stmt->execute();
        ...
        $stmt->bind_param("s", $text);
        $stmt->execute();
        sendmessage($from_id, $textbotlang['Admin']['managepanel']['addpanelurl'], $backadmin);
        $stmt = $connect->prepare("UPDATE user SET step = ? WHERE id = ?");
        $step = 'add_link_panel';
        $stmt->bind_param("ss", $step, $from_id);
        $stmt->execute();
        ...
        if (!filter_var($text, FILTER_VALIDATE_URL)) {
                    sendmessage($from_id, $textbotlang['Admin']['managepanel']['Invalid-domain'], $backadmin);
                    return;
    }
            sendmessage($from_id,$textbotlang['Admin']['managepanel']['usernameset'], $backadmin);
            $stmt = $connect->prepare("UPDATE user SET step = ? WHERE id = ?");
        ...
        sendmessage($from_id,$textbotlang['Admin']['managepanel']['getpassword'], $backadmin);
        $stmt = $connect->prepare("UPDATE user SET step = ? WHERE id = ?");
        ...
        sendmessage($from_id,$textbotlang['Admin']['managepanel']['addedpanel'], $backadmin);
    ...
    if ($text == "❌ حذف پنل") {
        sendmessage($from_id,$textbotlang['Admin']['managepanel']['GetRemoveNamePanel'],$json_list_marzban_panel);
        $stmt = $connect->prepare("UPDATE user SET step = ? WHERE id = ?");
        $step = 'removepanel';
        $stmt->bind_param("ss", $step, $from_id);
        $stmt->execute();
    } elseif ($user['step'] == "removepanel") {
        sendmessage($from_id,$textbotlang['Admin']['managepanel']['RemovedPanel'], $keyboardmarzban);
    ...
    if ($text == "📨 ارسال پیام به کاربر") {
        sendmessage($from_id, $textbotlang['users']['selectoption'], $sendmessageuser);
    } elseif ($text == "✉️ ارسال همگانی") {
        sendmessage($from_id, $textbotlang['Admin']['ManageUser']['GetText'], $backadmin);
        $stmt = $connect->prepare("UPDATE user SET step = ? WHERE id = ?");
        ...
        sendmessage($from_id,$textbotlang['Admin']['ManageUser']['ForwardGetext'], $backadmin);
        $stmt = $connect->prepare("UPDATE user SET step = ? WHERE id = ?");
        ...
        sendmessage($from_id,$textbotlang['Admin']['ManageUser']['ForwardSendAllUser'], $keyboardadmin);
        $stmt = $connect->prepare("UPDATE user SET step = ? WHERE id = ?");
        $step = 'home';
        $stmt->bind_param("ss", $step, $from_id);
        $stmt->execute();
    ...
    if ($text  == "📝 تنظیم متن ربات") {
        sendmessage($from_id,$textbotlang['users']['selectoption'], $textbot);
    ...
    elseif ($user['step'] == "changetextstart") {
        if(!$text){
                sendmessage($from_id,$textbotlang['Admin']['ManageUser']['ErrorText'], $textbot);
                return;
        }
        sendmessage($from_id, $textbotlang['Admin']['ManageUser']['SaveText'], $textbot);
    ...
    elseif ($text == "متن دکمه 📚 آموزش") {
        $textstart = $textbotlang['Admin']['ManageUser']['ChangeTextGet']. $datatextbot['text_help'];
        sendmessage($from_id, $textstart, $backadmin);
        $stmt = $connect->prepare("UPDATE user SET step = ? WHERE id = ?");
        $step = 'text_help';
        $stmt->bind_param("ss", $step, $from_id);
    ...
    elseif ($text == "متن دکمه ☎️ پشتیبانی") {
        $textstart = $textbotlang['Admin']['ManageUser']['ChangeTextGet']. $datatextbot['text_support'];
        sendmessage($from_id, $textstart, $backadmin);
        $stmt = $connect->prepare("UPDATE user SET step = ? WHERE id = ?");
        $step = 'text_support';
        $stmt->bind_param("ss", $step, $from_id);
    ...
    elseif ($text == "دکمه سوالات متداول") {
        $textstart = $textbotlang['Admin']['ManageUser']['ChangeTextGet']. $datatextbot['text_fq']."```";
        sendmessageMarkdown($from_id, $textstart, $backadmin);
        $stmt = $connect->prepare("UPDATE user SET step = ? WHERE id = ?");
        $step = 'text_fq';
        $stmt->bind_param("ss", $step, $from_id);
    ...
    elseif ($text == "📝 تنظیم متن توضیحات سوالات متداول") {
        $textstart = $textbotlang['Admin']['ManageUser']['ChangeTextGet']. $datatextbot['text_dec_fq']."```";
        sendmessageMarkdown($from_id, $textstart, $backadmin);
        $stmt = $connect->prepare("UPDATE user SET step = ? WHERE id = ?");
        $step = 'text_dec_fq';
        $stmt->bind_param("ss", $step, $from_id);
    ...
    elseif ($text == "📝 تنظیم متن توضیحات عضویت اجباری") {
        $textstart = $textbotlang['Admin']['ManageUser']['ChangeTextGet']. $datatextbot['text_channel']."```";
        sendmessageMarkdown($from_id, $textstart, $backadmin);
        $stmt = $connect->prepare("UPDATE user SET step = ? WHERE id = ?");
        $step = 'text_channel';
        $stmt->bind_param("ss", $step, $from_id);
    ...
    elseif ($text == "دکمه افزایش موجودی") {
        $textstart = $textbotlang['Admin']['ManageUser']['ChangeTextGet']. $datatextbot['text_Add_Balance']."```";
        sendmessageMarkdown($from_id, $textstart, $backadmin);
        $stmt = $connect->prepare("UPDATE user SET step = ? WHERE id = ?");
    ...
    elseif ($text == "📝 تنظیم متن توضیحات شماره کارت") {
        $textstart = $textbotlang['Admin']['ManageUser']['ChangeTextGet']. $datatextbot['text_cart_to_cart']."```";
        sendmessageMarkdown($from_id, $textstart, $backadmin);
        $stmt = $connect->prepare("UPDATE user SET step = ? WHERE id = ?");
    ...
    elseif ($text == "متن دکمه خرید اشتراک") {
        $textstart = $textbotlang['Admin']['ManageUser']['ChangeTextGet']. $datatextbot['text_sell'];
        sendmessage($from_id, $textstart, $backadmin);
        $stmt = $connect->prepare("UPDATE user SET step = ? WHERE id = ?");
        $step = 'text_sell';
        $stmt->bind_param("ss", $step, $from_id);
    ...
    elseif ($text == "متن دکمه لیست تعرفه") {
        $textstart = $textbotlang['Admin']['ManageUser']['ChangeTextGet']. $datatextbot['text_Tariff_list'];
        sendmessage($from_id, $textstart, $backadmin);
        $stmt = $connect->prepare("UPDATE user SET step = ? WHERE id = ?");
    ...
    elseif ($text == "متن توضیحات لیست تعرفه") {
        $textstart = $textbotlang['Admin']['ManageUser']['ChangeTextGet']. $datatextbot['text_dec_Tariff_list'];
        sendmessage($from_id, $textstart, $backadmin);
        $stmt = $connect->prepare("UPDATE user SET step = ? WHERE id = ?");
    ...
    elseif ($text ==  "دکمه حساب کاربری") {
        $textstart = $textbotlang['Admin']['ManageUser']['ChangeTextGet']. $datatextbot['text_Account_op'];
        sendmessage($from_id, $textstart, $backadmin);
        $stmt = $connect->prepare("UPDATE user SET step = ? WHERE id = ?");
    ...
    if ($text == "✍️ ارسال پیام برای یک کاربر") {
        ...
        $stmt->bind_param("ss", $text, $from_id);
        $stmt->execute();
        sendmessage($from_id, $textbotlang['Admin']['ManageUser']['GetIDMessage'], $backadmin);
        $stmt = $connect->prepare("UPDATE user SET step = ? WHERE id = ?");
        $step = 'sendmessagetid';
        $stmt->bind_param("ss", $step, $from_id);
        $stmt->execute();
        ...
        sendmessage($from_id,$textbotlang['Admin']['ManageUser']['MessageSent'], $keyboardadmin);
        $stmt = $connect->prepare("UPDATE user SET step = ? WHERE id = ?");
        $step = 'home';
        $stmt->bind_param("ss", $step, $from_id);
        $stmt->execute();
    ...
    //_________________________________________________
    if ($text == "📚 بخش آموزش"){
        sendmessage($from_id, $textbotlang['users']['selectoption'], $keyboardhelpadmin);
    }
    elseif ($text == "📚 اضافه کردن آموزش") {
        sendmessage($from_id, $textbotlang['Admin']['Help']['GetAddNameHelp'], $backadmin);
        $stmt = $connect->prepare("UPDATE user SET step = ? WHERE id = ?");
        $step = 'add_name_help';
        $stmt->bind_param("ss", $step, $from_id);
        $stmt->execute();
    ...
    elseif ($user['step'] == "add_name_help") {
        ...
        $stmt->bind_param("s", $text);
        $stmt->execute();
        sendmessage($from_id, $textbotlang['Admin']['Help']['GetAddDecHelp'], $backadmin);
        $stmt = $connect->prepare("UPDATE user SET step = ? WHERE id = ?");
        $step = 'add_dec';
        $stmt->bind_param("ss", $step, $from_id);
        $stmt->execute();
    ...
    elseif ($user['step'] == "add_dec") {
        ...
        sendmessage($from_id,$textbotlang['Admin']['Help']['SaveHelp'],$keyboardadmin);
        $stmt = $connect->prepare("UPDATE user SET step = ? WHERE id = ?");
        $step = 'home';
        $stmt->bind_param("ss", $step, $from_id);
        $stmt->execute();
    ...
    elseif ($text == "❌ حذف آموزش") {
        sendmessage($from_id, $textbotlang['Admin']['Help']['SelectName'], $json_list_help);
        $stmt = $connect->prepare("UPDATE user SET step = ? WHERE id = ?");
        $step = 'remove_help';
        $stmt->bind_param("ss", $step, $from_id);
        $stmt->execute();
    ...
    elseif ($user['step'] == "remove_help") {
        ...
        $stmt->bind_param("s", $text);
        $stmt->execute();
        sendmessage($from_id,$textbotlang['Admin']['Help']['RemoveHelp'] ,$keyboardhelpadmin);
        $stmt = $connect->prepare("UPDATE user SET step = ? WHERE id = ?");
        $step = 'home';
        $stmt->bind_param("ss", $step, $from_id);
        $stmt->execute();
    ...
    if(preg_match('/Response_(\w+)/',$datain, $dataget)) {
        ...
        $stmt->bind_param("ss", $step, $from_id);
        $stmt->execute();
        sendmessage($from_id,$textbotlang['Admin']['ManageUser']['GetTextResponse'], $backadin);
    }
    elseif($user['step'] == "getmessageAsAdmin"){
        sendmessage($from_id, $textbotlang['Admin']['ManageUser']['SendMessageuser'], null);
       $textSendAdminToUser = "
    ...
    if ($text == "📡 وضعیت ربات") {
        sendmessage($from_id,$textbotlang['Admin']['Status']['BotTitle'] ,$Bot_Status);
    }
    ...
    if ($text == "👤 دکمه نام کاربری") {
        sendmessage($from_id, $textbotlang['Admin']['Status']['UsernameTitle'], $not_user);
    }
    if ($datain == "onnotuser"){
    ...
    if ($text == "⚜️ دو ستونه محصول") {
        sendmessage($from_id,$textbotlang['Admin']['Status']['two_columnseTitle'] , $two_columns);
    }
    if ($datain == "on"){
    ...
    if ($text == "🔒 مسدود کردن کاربر") {
        sendmessage($from_id,$textbotlang['Admin']['ManageUser']['BlockUserId'], $backadmin);
        $stmt = $connect->prepare("UPDATE user SET step = ? WHERE id = ?");
        $step = 'getidblock';
        $stmt->bind_param("ss", $step, $from_id);
        $stmt->execute();
        ...
        if ($userblock['User_Status'] == "block") {
            sendmessage($from_id, $textbotlang['Admin']['ManageUser']['BlockedUser'], $backadmin);
            return;
        }
        $stmt = $connect->prepare("UPDATE user SET Processing_value = ? WHERE id = ?");
        $stmt->bind_param("ss", $text, $from_id);
        ...
        $stmt->execute();
        sendmessage($from_id, $textbotlang['Admin']['ManageUser']['BlockUser'], $backadmin);
        $stmt = $connect->prepare("UPDATE user SET step = ? WHERE id = ?");
        ...
        $stmt->bind_param("ss", $text, $Processing_value);
        $stmt->execute();
        sendmessage($from_id, $textbotlang['Admin']['ManageUser']['DescriptionBlock'],$keyboardadmin);
        $stmt = $connect->prepare("UPDATE user SET step = ? WHERE id = ?");
        $step = 'home';
        $stmt->bind_param("ss", $step, $from_id);
        $stmt->execute();
    } elseif ($text == "🔓 رفع مسدودی کاربر") {
        sendmessage($from_id, $textbotlang['Admin']['ManageUser']['GetIdUserunblock'], $backadmin);
        $stmt = $connect->prepare("UPDATE user SET step = ? WHERE id = ?");
        $step = 'getidunblock';
        $stmt->bind_param("ss", $step, $from_id);
        $stmt->execute();
        ...
        if ($userunblock['User_Status'] == "Active") {
            sendmessage($from_id, $textbotlang['Admin']['ManageUser']['UserNotBlock'] , $backadmin);
            return;
        }
        $stmt = $connect->prepare("UPDATE user SET User_Status = ? WHERE id = ?");
        $User_Status = "Active";
        ...
        $stmt->execute();
        sendmessage($from_id, $textbotlang['Admin']['ManageUser']['UserUnblocked'], $keyboardadmin);
        $stmt = $connect->prepare("UPDATE user SET step = ? WHERE id = ?");
        $step = 'home';
        $stmt->bind_param("ss", $step, $from_id);
        $stmt->execute();
    ...
    if($text == "♨️ بخش قوانین"){
        sendmessage($from_id, $textbotlang['users']['selectoption'], $rollkey);
    }
    elseif ($text == "⚖️ متن قانون") {
        $textstart = $textbotlang['Admin']['ManageUser']['ChangeTextGet']. $datatextbot['text_roll']."```";
        sendmessageMarkdown($from_id, $textstart, $backadmin);
        $stmt = $connect->prepare("UPDATE user SET step = ? WHERE id = ?");
        $step = 'text_roll';
        $stmt->bind_param("ss", $step, $from_id);
    ...
    if($text == "💡 روشن / خاموش کردن تایید قوانین"){
        sendmessage($from_id, $textbotlang['Admin']['Status']['rollTitle'], $roll_Status);
    }
    ...
    //_________________________________________________
    if($text == "👤 خدمات کاربر"){
        sendmessage($from_id, $textbotlang['users']['selectoption'], $User_Services);
    }
    #-------------------------#
    ...
    elseif ($user['step'] == "get_status"){
        ...
        if ($user_phone_status['number'] == "none"){
            sendmessage($from_id, $textbotlang['Admin']['phone']['notactive'], $User_Services);
        }
        else{
            sendmessage($from_id,$textbotlang['Admin']['phone']['active'],$User_Services);
        }
        $stmt = $connect->prepare("UPDATE user SET step = ? WHERE id = ?");
        $step = 'home';
        $stmt->bind_param("ss", $step, $from_id);
    ...
    if($text == "☎️ وضعیت احراز هویت شماره تماس"){
        sendmessage($from_id, $textbotlang['Admin']['Status']['phoneTitle'], $get_number);
    }
    ...
    elseif ($user['step'] == "get_number_admin"){
        ...
        if ($user_phone_number['number'] == "none"){
            sendmessage($from_id, $textbotlang['Admin']['phone']['NotSend'], $User_Services);
            return;
        }
        $text_number = "
    ...
    if($text == "📣 تنظیم کانال گزارش"){
        $text_channel = $textbotlang['Admin']['Channel']['ReportChannel'] . $setting['Channel_Report'];
    ...
    elseif ($user['step'] == "addchannelid") {
        sendmessage($from_id, $textbotlang['Admin']['Channel']['SetChannelReport'], $keyboardadmin);
        ...
        $stmt->bind_param("ss", $step, $from_id);
        $stmt->execute();
        sendmessage($setting['Channel_Report'], $textbotlang['Admin']['Channel']['TestChannel'], null);
    }
    #-------------------------#
    if($text == "🏬 بخش فروشگاه"){
        sendmessage($from_id,$textbotlang['users']['selectoption'], $shopkeyboard);
    }
    elseif ($text == "🛍 اضافه کردن محصول"){
        sendmessage($from_id, $textbotlang['Admin']['Product']['AddProductStepOne'], $backadmin);
        $stmt = $connect->prepare("UPDATE user SET step = ? WHERE id = ?");
        $step = 'get_limit';
        $stmt->bind_param("ss", $step, $from_id);
        $stmt->execute();
    ...
    elseif($user['step'] == "get_limit"){
        ...
        $stmt->bind_param("ss", $text, $from_id);
        $stmt->execute();
        sendmessage($from_id,$textbotlang['Admin']['Product']['GetLimit'], $backadmin);
        $stmt = $connect->prepare("UPDATE user SET step = ? WHERE id = ?");
        $step = 'get_time';
        $stmt->bind_param("ss", $step, $from_id);
        $stmt->execute();
    ...
    elseif($user['step'] == "get_time"){
        if (!ctype_digit($text))
        {
            sendmessage($from_id,$textbotlang['Admin']['Product']['Invalidvolume'], $backadmin);
            return;
        }
        $stmt = $connect->prepare("UPDATE product SET Volume_constraint = ? WHERE name_product = ?");
        $stmt->bind_param("ss", $text, $Processing_value);
        $stmt->execute();
        sendmessage($from_id,$textbotlang['Admin']['Product']['GettIime'], $backadmin);
        $stmt = $connect->prepare("UPDATE user SET step = ? WHERE id = ?");
        $step = 'get_price';
        $stmt->bind_param("ss", $step, $from_id);
        $stmt->execute();
    ...
    elseif($user['step'] == "get_price"){
        if (!ctype_digit($text))
        {
            sendmessage($from_id,$textbotlang['Admin']['Product']['InvalidTime'], $backadmin);
            return;
        }
        $stmt = $connect->prepare("UPDATE product SET Service_time = ? WHERE name_product = ?");
        $stmt->bind_param("ss", $text, $Processing_value);
        $stmt->execute();
        sendmessage($from_id,$textbotlang['Admin']['Product']['GetPrice'], $backadmin);
        $stmt = $connect->prepare("UPDATE user SET step = ? WHERE id = ?");
        $step = 'endstep';
        $stmt->bind_param("ss", $step, $from_id);
        $stmt->execute();
    ...
    elseif ($user['step'] == "endstep"){
        if (!ctype_digit($text))
        {
            sendmessage($from_id,$textbotlang['Admin']['Product']['InvalidPrice'],$backadmin);
            return;
        }
        $stmt = $connect->prepare("UPDATE product SET price_product = ? WHERE name_product = ?");
        $stmt->bind_param("ss", $text, $Processing_value);
        $stmt->execute();
        sendmessage($from_id,$textbotlang['Admin']['Product']['SaveProduct'], $shopkeyboard);
        $stmt = $connect->prepare("UPDATE user SET step = ? WHERE id = ?");
        $step = 'home';
        $stmt->bind_param("ss", $step, $from_id);
        $stmt->execute();
    ...
    #-------------------------#
    if($text == "👨‍🔧 بخش ادمین"){
        sendmessage($from_id,$textbotlang['users']['selectoption'],$admin_section_panel);
    }
    #-------------------------#
    if($text == "⚙️ تنظیمات"){
        sendmessage($from_id,$textbotlang['users']['selectoption'],$setting_panel);
    }
    #-------------------------#
    if($text == "📱 احراز هویت شماره"){
        sendmessage($from_id,$textbotlang['users']['selectoption'],$valid_Number);
    }
    #-------------------------#
    if($text == "📊 بخش گزارشات"){
        sendmessage($from_id,$textbotlang['users']['selectoption'],$reports);
    }
    #-------------------------#
    if($text == "🔑 تنظیمات اکانت تست"){
        sendmessage($from_id,$textbotlang['users']['selectoption'],$keyboard_usertest);
    }
    #-------------------------#
    ...
    if(preg_match('/reject_pay_(\w+)/',$datain, $datagetr)) {
        ...
        sendmessage($from_id,$textbotlang['Admin']['Payment']['Reasonrejecting'],$backadmin);
        $stmt = $connect->prepare("UPDATE user SET step = ? WHERE id = ?");
        $step = "reject-dec";
    ...
    elseif($user['step'] == "reject-dec"){
        ...
        sendmessage($from_id,$textbotlang['Admin']['Payment']['Rejected'],$keyboardadmin);
        $stmt = $connect->prepare("UPDATE user SET step = ? WHERE id = ?");
        $step = "home";
        $stmt->bind_param("ss", $step, $from_id);
        $stmt->execute();
    ...
    elseif($user['step'] == "change_val"){
        ...
        $stmt->bind_param("ss", $text, $Processing_value);
        $stmt->execute();
        sendmessage($from_id,$textbotlang['Admin']['Product']['volumeUpdated'],$shopkeyboard);
        $stmt = $connect->prepare("UPDATE user SET step = ? WHERE id = ?");
        $step = 'home';
        $stmt->bind_param("ss", $step, $from_id);
        $stmt->execute();
    ...
    #-------------------------#
    if($text == "زمان"){
        sendmessage($from_id,$textbotlang['Admin']['Product']['NewTime'],$backadmin);
        $stmt = $connect->prepare("UPDATE user SET step = ? WHERE id = ?");
        $step = 'change_time';
        $stmt->bind_param("ss", $step, $from_id);
        $stmt->execute();
    ...
    elseif($user['step'] == "change_time"){
        ...
        $stmt->bind_param("ss", $text, $Processing_value);
        $stmt->execute();
        sendmessage($from_id,$textbotlang['Admin']['Product']['TimeUpdated'],$shopkeyboard);
        $stmt = $connect->prepare("UPDATE user SET step = ? WHERE id = ?");
        $step = 'home';
        $stmt->bind_param("ss", $step, $from_id);
        $stmt->execute();
    ...
    elseif ($user['step'] == "updatetime"){
        ...
        $stmt->bind_param("s", $text);
        $stmt->execute();
        sendmessage($from_id,$textbotlang['Admin']['Usertest']['TimeUpdated'],$keyboard_usertest);
        $stmt = $connect->prepare("UPDATE user SET step = ? WHERE id = ?");
        $step = 'home';
        $stmt->bind_param("ss", $step, $from_id);
        $stmt->execute();
    ...
    elseif ($user['step'] == "val_usertest"){
        ...
        $stmt->bind_param("s", $text);
        $stmt->execute();
        sendmessage($from_id,$textbotlang['Admin']['Usertest']['VolumeUpdated'],$keyboard_usertest);
        $stmt = $connect->prepare("UPDATE user SET step = ? WHERE id = ?");
        $step = 'home';
        $stmt->bind_param("ss", $step, $from_id);
        $stmt->execute();
    ...
    if ($text == "⬆️️️ افزایش موجودی کاربر") {
        sendmessage($from_id, $textbotlang['Admin']['Balance']['AddBalance'], $backadmin);
        $stmt = $connect->prepare("UPDATE user SET step = ? WHERE id = ?");
        $step = 'add_Balance';
        $stmt->bind_param("ss", $step, $from_id);
        $stmt->execute();
    ...
    elseif ($user['step'] == "add_Balance") {
        ...
        sendmessage($from_id,$textbotlang['Admin']['Balance']['PriceBalance'], $backadmin);
        $stmt = $connect->prepare("UPDATE user SET Processing_value = ? WHERE id = ?");
        $stmt->bind_param("ss", $text, $from_id);
        $stmt->execute();
        $stmt = $connect->prepare("UPDATE user SET step = ? WHERE id = ?");
    ...
    elseif ($user['step'] == "get_price_add") {
        if (!ctype_digit($text))
        {
            sendmessage($from_id,$textbotlang['Admin']['Balance']['Invalidprice'], $backadmin);
            return;
        }
        sendmessage($from_id,$textbotlang['Admin']['Balance']['AddBalanceUser'] ,$User_Services);
    ...
    if ($text == "⬇️ کم کردن موجودی") {
        sendmessage($from_id, $textbotlang['Admin']['Balance']['NegativeBalance'], $backadmin);
        $stmt = $connect->prepare("UPDATE user SET step = ? WHERE id = ?");
    ...
    elseif ($user['step'] == "Negative_Balance") {
        ...
        sendmessage($from_id, $textbotlang['Admin']['Balance']['PriceBalancek'] , $backadmin);
        $stmt = $connect->prepare("UPDATE user SET Processing_value = ? WHERE id = ?");
        $stmt->bind_param("ss", $text, $from_id);
        $stmt->execute();
        $stmt = $connect->prepare("UPDATE user SET step = ? WHERE id = ?");
    ...
    elseif ($user['step'] == "get_price_Negative") {
        if (!ctype_digit($text))
        {
            ...
            sendmessage($from_id,$textbotlang['Admin']['Balance']['NegativeBalanceUser'],$User_Services);
        $Balance_user = mysqli_fetch_assoc(mysqli_query($connect, "SELECT * FROM user WHERE id = '$Processing_value' LIMIT 1"));
    ...
    elseif ($user['step'] == "show_info"){
        ...
        sendmessage($from_id, $textbotlang['Admin']['ManageUser']['ViewInfo'], $userinfo);
        sendmessage($from_id,$textbotlang['users']['selectoption'], $User_Services);
        $stmt = $connect->prepare("UPDATE user SET step = ? WHERE id = ?");
        $step = 'home';
        $stmt->bind_param("ss", $step, $from_id);
    ...
    if ($text == "💡 وضعیت بخش آموزش") {
        sendmessage($from_id,$textbotlang['Admin']['Status']['HelpTitle'],$help_Status);
    }
    ...
    if($text == "🎁 ساخت کد هدیه"){
        sendmessage($from_id, $textbotlang['Admin']['Discount']['GetCode'], $backadmin);
        $stmt = $connect->prepare("UPDATE user SET step = ? WHERE id = ?");
        $step = 'get_code';
        $stmt->bind_param("ss", $step, $from_id);
        $stmt->execute();
    ...
    elseif($user['step'] == "get_code"){
        if (!preg_match('/^[A-Za-z]+$/', $text)) {
                sendmessage($from_id,$textbotlang['Admin']['Discount']['ErrorCode'],null);
                return;
    }
        $stmt = $connect->prepare("INSERT INTO Discount (code) VALUES (?)");
        $stmt->bind_param("s", $text);
        $stmt->execute();
        sendmessage($from_id,$textbotlang['Admin']['Discount']['PriceCode'],null);
        $stmt = $connect->prepare("UPDATE user SET step = ? WHERE id = ?");
        $step = 'get_price_code';
        $stmt->bind_param("ss", $step, $from_id);
        $stmt->execute();
    ...
    elseif($user['step'] == "get_price_code"){
        ...
        $stmt->bind_param("ss", $text, $Processing_value);
        $stmt->execute();
        sendmessage($from_id,$textbotlang['Admin']['Discount']['SaveCode'],$keyboardadmin);
        $stmt = $connect->prepare("UPDATE user SET step = ? WHERE id = ?");
        $step = 'home';
        $stmt->bind_param("ss", $step, $from_id);
        $stmt->execute();
    ...
    if ($text == "تایید شماره ایرانی 🇮🇷") {
        sendmessage($from_id, $textbotlang['Admin']['Status']['PhoneIranTitle'],$getNumberIran);
    }
    ...
    if ($text == "🔗 ارسال لینک سابسکرایبشن") {
        sendmessage($from_id, $textbotlang['Admin']['Status']['subTitle'],$sublinkkeyboard);
    }
    ...
    if ($text == "⚙️ارسال کانفیگ") {
        sendmessage($from_id, $textbotlang['Admin']['Status']['configTitle'],$configkeyboard);
    }
    ...
    if($text == "🛍 مشاهده سفارشات کاربر"){
        sendmessage($from_id,$textbotlang['Admin']['ManageUser']['ViewOrder'] ,$backadmin);
        $stmt = $connect->prepare("UPDATE user SET step = ? WHERE id = ?");
        $step = 'GetIdAndOrdedr';
        $stmt->bind_param("ss", $step, $from_id);
        $stmt->execute();
    ...
    📆 تاریخ خرید : $datatime
            ...
            sendmessage($from_id, $textbotlang['Admin']['ManageUser']['SendOrder'], $User_Services);
    }
    #----------------[  remove Discount   ]------------------#
    if ($text == "❌ حذف کد هدیه"){
        sendmessage($from_id,$textbotlang['Admin']['Discount']['RemoveCode'],$json_list_Discount_list_admin);
        $stmt = $connect->prepare("UPDATE user SET step = ? WHERE id = ?");
        ...
        if (!in_array($text , $code_Discount)){
            sendmessage($from_id, $textbotlang['Admin']['Discount']['NotCode'], null);
            return;
        }
        $stmt = $connect->prepare("DELETE FROM Discount WHERE code = ?");
        $stmt->bind_param("s", $text);
        $stmt->execute();
        sendmessage($from_id,$textbotlang['Admin']['Discount']['RemovedCode'],$shopkeyboard);
    }
    #----------------[  MANAGE protocol   ]------------------#
    if($text == "🌏 مدیریت پروتکل"){
         sendmessage($from_id,$textbotlang['Admin']['Protocol']['Title'],$keyboardprotocol);
        $stmt = $connect->prepare("UPDATE user SET step = ? WHERE id = ?");
        $step = 'Add_protocol';
        $stmt->bind_param("ss", $step, $from_id);
        $stmt->execute();
    ...
    elseif($user['step'] == "Add_protocol"){
        ...
        if(!in_array($text, $protocolch)){
            sendmessage($from_id,$textbotlang['Admin']['Protocol']['invalidProtocol'],null);
            return;
        }
        $connect->query("INSERT IGNORE INTO protocol (NameProtocol) VALUES ('$text')");
                sendmessage($from_id,$textbotlang['Admin']['Protocol']['AddedProtocol'],null);
    
    }
    #----------------[  REMOVE protocol   ]------------------#
    if ($text == "🗑 حذف پروتکل"){
        sendmessage($from_id,$textbotlang['Admin']['Protocol']['RemoveProtocol'], $keyboardprotocol);
        $stmt = $connect->prepare("UPDATE user SET step = ? WHERE id = ?");
        $step = 'removeprotocol';
        $stmt->bind_param("ss", $step, $from_id);
        $stmt->execute();
        ...
        sendmessage($from_id, $textbotlang['Admin']['Protocol']['RemovedProtocol'], $keyboardmarzban);
    ...
    if($text == "❌ حذف سرویس کاربر"){
        sendmessage($from_id, $textbotlang['Admin']['ManageUser']['RemoveService'] , $backadmin);
        $stmt = $connect->prepare("UPDATE user SET step = ? WHERE id = ?");
        $step = 'removeservice';
        $stmt->bind_param("ss", $step, $from_id);
        $stmt->execute();
    ...
    elseif($user['step'] == "removeservice"){
        ...
        $stmt->bind_param("s", $text);
        $stmt->execute();
        sendmessage($from_id,$textbotlang['Admin']['ManageUser']['RemovedService'], $keyboardadmin);
            $stmt = $connect->prepare("UPDATE user SET step = ? WHERE id = ?");
        $step = 'home';
        $stmt->bind_param("ss", $step, $from_id);
        $stmt->execute();
